...
**Note** that the
[`learn()`](https://pglpm.github.io/prova/reference/learn.md) function
that produced `Kexample` was called with the option to create only a
limited number (225) of Monte Carlo samples, in order to reduce its
...
hyperparameters saved in `Kexample`. It is only meant to be used for
illustration purposes of the package's capabilities.
