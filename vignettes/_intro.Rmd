...
```{r load_shuffled, echo = FALSE}
penguins_shuffled <- pread.csv('penguins_shuffled.csv')
```
...
```{r, echo = FALSE}
dat <- pread.csv('meta_penguins_temp.csv')
...
knitr::kable(meta_penguins, row.names = FALSE)
```
...
- Adélie: `r printprob(Fspecies10$value['Adelie',1])`
- Chinstrap: `r printprob(Fspecies10$value['Chinstrap',1])`
- Gentoo: `r printprob(Fspecies10$value['Gentoo',1])`
...
```{r vis10bis, eval = TRUE, out.width = '75%', fig.show = 'hold', fig.cap='**Uncertainty of relative frequencies displayed as quantile bands and samples**'}
plot(Fspecies10, col = 8)
plot(Fspecies10, col = 8, spread = 'samples')
```

The visualization by quantile bands often looks neater; but keep in mind that it leaves out important details, such as the presence of probable peaks; such details are instead shown by the ensemble visualization. For example in the ensemble plot above we see that the long-run frequency distribution is likely to have a maximum at the Chinstrap species (∧-shaped lines), or a mininum at the Chinstrap species (∨-shaped lines), and less likely to have a maximum or minimum at the other two species (which would appear as like /-shaped or \\-shaped lines). This aspect is not visible in the quantile-band plot. It is recommended to always take a look at both kinds of visualization. In the following we shall use only quantile bands for simplicity, but feel free to call all following `plot()`s also with the `spread = 'samples'` argument.
...
Note how, according to these preliminary estimates, the frequencies of Adélie and Chinstrap are very close, around `r printprob(mean(c(Fspecies10I$value['Adelie','Biscoe'], Fspecies10I$value['Chinstrap','Biscoe'])),1)`, and quite different from the frequency of Gentoo, around `r printprob(Fspecies10I$value['Gentoo','Biscoe'],1)`. Yet, the probability distributions are wide; each distribution gives a non-zero probability to all three estimates.
...
Among other things we see here a larger difference in the estimated frequencies of Adélie, around `r printprob(Fspecies10I$value['Adelie','Dream'])`, and Chinstrap, around `r printprob(Fspecies10I$value['Chinstrap','Dream'])`. Still, the probability distributions are wide.
...
We see that penguin #1 might have been female with probability `r printprob(imputeddata$value['female',1])`, although further data might shift that probability somewhere between `r printprob(imputeddata$quantiles['female',1,1])` and `r printprob(imputeddata$quantiles['female',1,4])`.
...
<!-- commented
The bill length of penguin #6 can be estimated with a median of around `r printprob(qPr(Yname = 'bill_len', p = 0.5, X = X, K = K10)$value[1,1])`.
commented -->
...
On Biscoe, the new frequency estimates for `Adelie` (solid blue line) and `Chinstrap` (dashed red line) are now around `r printprob(abs(diff(Fspecies60I$value[c('Adelie','Chinstrap'),'Biscoe'])),1)` apart, whereas the previous estimates were almost equal. Again this is not unexpected, and these changes are well within the previous credibility intervals.
...
> 
> Adélie
> : estimate `r printprob(Fspeciesall$value['Adelie',1])`
...
> 
> Chinstrap
> : estimate `r printprob(Fspeciesall$value['Chinstrap',1])`
...
> 
> Gentoo
> : estimate `r printprob(Fspeciesall$value['Gentoo',1])`
...
> 
> Adélie
> : estimate `r printprob(Fanalysis$value['Adelie',1])`
...
> 
> Chinstrap
> : estimate `r printprob(Fanalysis$value['Chinstrap',1])`
...
> 
> Gentoo
> : estimate `r printprob(Fanalysis$value['Gentoo',1])`
...
> 
> Biscoe
> : estimate `r printprob(Fanalysis$value['Biscoe',1])`
...
> 
> Dream
> : estimate `r printprob(Fanalysis$value['Dream',1])`
...
> 
> Torgersen
> : estimate `r printprob(Fanalysis$value['Torgersen',1])`
...
```{r, eval = TRUE, fig.cap='', fig.show = 'hold'}
