...
<!-- commented
The bill length of penguin #6 can be estimated with a median of around `r printprob(qPr(Yname = 'bill_len', p = 0.5, X = X, K = K10, parallel = 4)$values[1,1])`.
commented -->
...
The new frequency estimates for `Adelie` (solid red line) and `Chinstrap` (dashed blue line) are now around `r printprob(abs(diff(Fspecies60I$values[c('Adelie','Chinstrap'),'Biscoe'])),1)` apart, whereas the previous estimates were almost equal. Again this is not unexpected, and these changes are well within the previous credibility intervals.
