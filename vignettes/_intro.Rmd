...
The new frequency estimates for `Adelie` (solid red line) and `Chinstrap` (dashed blue line) are now around `r printprob(abs(diff(Fspecies60I$value[c('Adelie','Chinstrap'),'Biscoe'])),1)` apart, whereas the previous estimates were almost equal. Again this is not unexpected, and these changes are well within the previous credibility intervals.
...
If we created a parallel cluster at the beginning of our analysis, let's close it now.

```{r stopcluster, eval = TRUE}
