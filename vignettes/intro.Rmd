...
and can be read from the `values` element of the `Fspecies10` object, either directly or by using the `print()` function, which by default a number of significant digits corresponding to the Monte Carlo numerical accuracy:
...
# Error in parse(text = input): <text>:9:43: unexpected ','
# 8: 
# 9: Fspecies10IS <- Pr(Y = Y, X = X2, K = K10),
#                                              ^
```
