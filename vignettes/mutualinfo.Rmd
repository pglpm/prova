...
Between variates `island` and `species` there is thus a mutual information of 0.61 Sh. But what does this mean?
...
which is different from the rough $r$-equivalent 0.68.
...
## Uncertainty about associations and variability of mutual information

Any measure of association that we calculate from a finite sample of data is obviously uncertain: a much larger data sample might reveal a stronger or weaker association. Likewise, the mutual association that we calculate from a finite sample of data is subject to a possible variability, in the sense that if we collected many more data, and calculated the mutual association from them, we could find a slightly different -- or maybe the same -- value.

The function `mutualinfo()` calculates not only the mutual information between two groups of variates from the present dataset. It also calculates the possible *variability* of the mutual information, if more data were collected, together with the probability of such a variation.
...
which shows that, acquiring more data, we can expect with 89% probability a mutual information between 0.15 Sh and 1 Sh. We can also visualize our uncertainty about this "long-run" mutual information by means of a probability distribution, using the `hist()` function:
...
<div class="figure">
<img src="figure/histislandspecies-1.jpeg" alt="**Variability of mutual information with larger data samples**" width="100%" />
<p class="caption">**Variability of mutual information with larger data samples**</p>
</div>
