...
```{r loadpenguins, eval = TRUE, echo = TRUE}
if(!exists('penguins')){
...
Between variates `island` and `species` there is thus a mutual information of `r signif(MIislandspecies$value, 2)` Sh. But what does this mean?
...
```{r variabilityislandspecies, eval = TRUE}
print(MIislandspecies)
```

which shows that, acquiring more data, we can expect with 89% probability a mutual information between `r signif(MIislandspecies$quantiles[1], 2)` Sh and `r signif(MIislandspecies$quantiles[4], 2)` Sh. We can also visualize our uncertainty about this "long-run" mutual information by means of a probability distribution, using the `hist()` function:

```{r histislandspecies, eval = TRUE, fig.cap='**Revisability of mutual information with a much larger data sample**'}
